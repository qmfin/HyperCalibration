...
def generate_underlying_path_by_rbergomi(T, S0, H, rho, eta, v0, n=365, N=10000):
    a= (2*H-1)/2
    ...
    V = rB.V(Y, xi = v0, eta = eta)
    S = rB.S(V, dB, S0)
    return S


def rb_pricing(input_all, S0, H, rho, eta, v0, n=365, N=10000):
    K, T, C = input_all.T
    t = T.max() + 1.0/n
    underlying_path = generate_underlying_path_by_rbergomi(t, S0, H, rho, eta, v0, n, N)
    slice_index = [int(np.round(i*365)) for i in T]
    ST = underlying_path[:,slice_index]
    payoffs = np.maximum((ST-K[None,:])*C,0)
    prices = np.mean(payoffs, axis=0)
    results = {'param': np.array([H, rho, eta, v0]), 
               'input': input_all, 
               'output': prices.reshape(-1,1)}
    return results
